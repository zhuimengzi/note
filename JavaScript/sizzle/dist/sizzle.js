...
function Sizzle( selector, context, results, seed ) {
	...
	// 尝试在HTML文档中快速找到操作（而​​不是过滤器）
	if ( !seed ) {

		// 如果根节点不等于document，则调用setDocument设置合理的根元素
		if ( ( context ? context.ownerDocument || context : preferredDoc ) !== document ) {
			setDocument( context );
		}
		// 确定上下文环境
		context = context || document;
		// 如果document.documentElement是html
		if ( documentIsHTML ) {

			// 如果选择器比较简单则直接使用 "get*By*" 系列方法
			// 上下文不等于DocumentFragment并且选择器为id或tag或class时
				...
				if ( (m = match[1]) ) {
					...
					if ( nodeType === 9 ) {
						...
						if ( (elem = context.getElementById( m )) ) {
							...
							// 如果元素存在并且元素的id等于正则匹配时的id则将此元素加入到results中并返回
							if ( elem.id === m ) {
								results.push( elem );
								return results;
							}
...
isXML = Sizzle.isXML = function( elem ) {
	// 如果documentElement存在并且为HTML则返回false，否则true
